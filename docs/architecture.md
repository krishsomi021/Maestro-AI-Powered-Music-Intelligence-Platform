...
Endpoints:
- `POST /jobs` — validate payload, persist job as `pending`, enqueue task, return `job_id`
...
stateDiagram-v2
    ...
    running --> failed : exception — error written
    complete --> [*]
    failed --> [*]
```
...
1. **Submit** — Client sends `POST /jobs` with `job_type` and `payload`
2. **Validate** — FastAPI validates payload shape with Pydantic v2
3. **Persist** — FastAPI writes job row to Postgres with `status = pending`, generates UUID `job_id`
4. **Enqueue** — FastAPI sends Celery task to Redis with the `job_id`
5. **Respond** — FastAPI returns `job_id` to client immediately — request is complete
6. **Pick up** — Free Celery worker pulls task from Redis, sets `status = running`, sets `started_at`
7. **Execute** — Worker runs task logic (real inference or mocked work)
8. **Finalize** — Worker writes result or error, sets final status and `completed_at`, acknowledges task to Redis
9. **Retrieve** — Client polls `GET /jobs/{id}` until status is `complete` or `failed`
